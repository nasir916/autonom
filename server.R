###############################################
# Dashboard Automated Statistical Analytics 
# Aims: To provide a platform for automated statistical analysis
#     : focus on logistic regression and nomogram
###############################################
...
shinyServer(
  ...
  function(input, output) {
    ...
    output$modified_df_name <- renderUI({ 
      textInput("ModifiedDfName", "10. Enter the data frame name.", 
                value= "mod_df")     
    })
    ...
    output$View_main_df <- renderUI({ 
      selectInput("viewMainDF", "Want to view the top of the model data? (Default: mtcars)", 
                  choices = c("No", "Yes"), multiple=FALSE, selected="No")
    })
    ...
    output$datastr_txt1 <- renderPrint({
    ...
    output$approximate_print <- DT::renderDT({
      apx_mdl()[["apx_rslt"]]  
    })
    ...
    output$BeginModel <- renderUI({  
      selectInput("begin_mdl", "14. Begin modeling?", 
                  choices = c("No", "Yes"), multiple=FALSE, selected="No")     #Will make choices based on my reactive function.
    })
    ...
    output$reg_typ <- renderUI({                                #Creates a UI function here but it will
      selectInput("regress_type", "2. Select the regression method",
                  choices = c("Linear", 
                              "Logistic", 
    ...
    output$censoring <- renderUI({                                 #Same idea as output$vy
      textInput("censor", "9. Censoring variable for survival model")     #Will make choices based on my reactive function.
    })
    
    #7. Indicate if there should be splines.
    output$aft_dist <- renderUI({  
      selectInput("aftDist", "11. Select the Y distribution for AFT models", choices = c("weibull", "exponential", "gaussian","logistic","lognormal", "loglogistic"), 
    ...
    output$clustering <- renderUI({                                 #Same idea as output$vy
      selectInput("gls_clst", "12. GLS clustering: List level 1 and 2 variables (e.g., time | id)",     #Will make choices based on my reactive function.
    ...
    output$quant_tau <- renderUI({                                 #Same idea as output$vy
      numericInput("tau1", "13. Quantile level for quantile regression",
    ...
    output$rcs_yes <- renderUI({                                 #Same idea as output$vy
      selectInput("rcsy", "8. Do you want splines?", 
                  choices = c("No", "Yes"), multiple=FALSE, selected="No")     #Will make choices based on my reactive function.
    })
    
    #Select the variables that will have splines.
    output$rx <- renderUI({                                 #Same idea as output$vy
      selectInput("rcs_X", "7. Select the spline variables (continuous only)", 
    ...
    output$update_yes <- renderUI({                                 #Same idea as output$vy
      selectInput("updy", "5. Do you want to update the model formula?", 
                  choices = c("No", "Yes"), multiple=FALSE, selected="No")     #Will make choices based on my reactive function.
    })
    
    #13. Update the model formula.
    output$uf <- renderUI({                                 #Same idea as output$vy
      textInput("up_fmla", "6. Update formula (Interactions: Change \"+\" to \'*\', strat(X) to stratify CPH).", 
    ...
    output$nomo_up_Fmla <- renderUI({
      textInput("nomoUpFmla", "1. Update nomogram formula", 
    ...
    output$nomo_yes <- renderUI({                                 #Same idea as output$vy
      selectInput("nomoYes", "2. Do you want to create a new nomogram?", 
                  choices = c("No", "Yes"), multiple=FALSE, selected="No")     #Will make choices based on my reactive function.
    })
